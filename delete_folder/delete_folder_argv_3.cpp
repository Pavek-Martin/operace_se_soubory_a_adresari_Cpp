...
int main(int argc, char **argv) {
	...
	cout<<"pouziti : delete_folder_argv_3.exe R:\\adresar_1"<<endl; // zadava se v cmd jedno lomitko
	system("pause");
	exit(1);
	}
    ...
    //const char* cesta = "/cesta/k/adresari"; // Nahraïte skuteènou cestou k adresáøi.
    //const char* cesta = "R:\\Nová složka"; // ZDE MENIT ! pozna soubor a adresar
    //const char* cesta = "R:\\abcd"; // ZDE MENIT !
    
    if (smaz_adresar(cesta) == 0) {
    cout<<"adresar "<<'"'<<cesta<<'"'<<" byl uspesne odstranen"<<endl;
    } else {
    cout<<"nastala chyba pri mazani adresare "<<'"'<<cesta<<'"'<<endl; // pozna soubor od adresare
    system("pause");
	exit(1);
    }
